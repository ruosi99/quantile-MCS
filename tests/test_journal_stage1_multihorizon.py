...
def test_graph_patchtst_quantile_output_shape_ordering_and_batch_safety():
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    adjacency = torch.tensor(
        [
            [1.0, 0.4, 0.0],
...
def test_stage1_build_model_can_select_graph_patchtst_quantile():
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    adjacency = torch.eye(3, dtype=torch.float32, device=device).to_sparse()
    args = SimpleNamespace(
    ...
    assert isinstance(model, models.GraphPatchTSTQuantile)
