...
# Local imports
from utils.model_training.baselines.layers import (
    ModernTCN_RevIN,
    ModernTCN_Stage,
...
class Arima:
    """
    ARIMA model predictor with automated order selection (without pmdarima).
    
    This class implements an ARIMA model using statsmodels only.
    It performs a grid search over (p, d, q) in a small range to find
    ...
    def predict(self, train_valid_feat, test_feat):
        """
        Perform predictions using ARIMA with manual auto-order selection.
        ...
        train_valid_feat = train_valid_feat[:-self.pred_len, :]  # Remove future period

        # Suppress convergence and estimation warnings
        warnings.filterwarnings("ignore", category=UserWarning)
        warnings.filterwarnings("ignore", category=RuntimeWarning)

        def fit_predict(j):
            fit_series = np.nan_to_num(train_valid_feat[:, j])
            if np.std(fit_series) < 1e-6:
                return [fit_series[-1]] * time_len

            best_aic = float('inf')
            best_model = None
            for p, d, q in itertools.product(range(0, 2), range(0, 2), range(0, 2)):
                try:
                    model_tmp = ARIMA(fit_series, order=(p, d, q),
                                    enforce_stationarity=False,
                                    enforce_invertibility=False)
                    fitted_tmp = model_tmp.fit()
                    if fitted_tmp.aic < best_aic:
                        best_aic = fitted_tmp.aic
                        best_model = fitted_tmp
                except:
                    continue
            ...
            return list(best_model.forecast(steps=time_len))

        # Parallel over nodes
        preds = Parallel(n_jobs=10)(delayed(fit_predict)(j) for j in range(node))
        preds = np.array(preds).T  # [T, N]
        return preds
...
class Fcnn(nn.Module):
    ...
    def forward(self, feat, extra_feat=None):
        ...
        if extra_feat is not None:
            ...
            x = torch.cat([feat.unsqueeze(-1), extra_feat.unsqueeze(-1)], dim=-1)
...
class GCN(nn.Module):
    def __init__(self, seq, n_fea, adj_dense):
        ...
        self.time_out = seq - n_fea + 1

        # 图归一化（假设 adj_dense 已含自环）
        ...
        self.register_buffer('A', a_delta)

        # 时间卷积：输入2个特征（occ, prc），输出1通道
        self.encoder = nn.Conv1d(in_channels=2, out_channels=1, kernel_size=n_fea)
        
        # 注意：Linear 作用于最后一维（时间维）
        self.gcn_l1 = nn.Linear(self.time_out, self.time_out)
        self.gcn_l2 = nn.Linear(self.time_out, self.time_out)
        self.decoder = nn.Linear(self.time_out, 1)
        self.act = nn.ReLU()

    def forward(self, occ, prc):
        B, N, S = occ.shape  # e.g., [16, 1445, 24]
        # print(f"Input occ shape: {occ.shape}")

        # [B, N, 2, S]
        x = torch.stack([occ, prc], dim=2)
        # print(f"After stack: {x.shape}")

        # [B*N, 2, S]
        x = x.view(B * N, 2, S)
        # print(f"After view to B*N: {x.shape}")

        # Conv1d on time axis
        x = self.encoder(x)  # [B*N, 1, T]
        # print(f"After Conv1d: {x.shape}")

        x = x.squeeze(1)    # [B*N, T]
        # print(f"After squeeze: {x.shape}")

        x = x.view(B, N, self.time_out)  # [B, N, T]
        # print(f"After final view: {x.shape}")  # 应该是 [16, 1445, 23]

        # --- GCN Layer 1 ---
        x = self.gcn_l1(x)  # [B, N, T] → Linear on T
        # print(f"After gcn_l1: {x.shape}")
        x = torch.matmul(self.A, x)  # [N,N] x [B,N,T] -> [B,N,T]
        x = self.act(x)
...
class TPA(nn.Module):
    def __init__(self, seq_len, n_fea, nodes, hidden_size=2):
        super().__init__()
        self.seq_len = seq_len
        self.nodes = nodes
        self.hidden_size = hidden_size

        # 如果只是拼接两个特征，不需要 Conv2d
        # 直接进入 LSTM
        self.lstm = nn.LSTM(input_size=2, hidden_size=hidden_size, 
                            num_layers=2, batch_first=True)
        
        # TPA attention layers
    ...
    def forward(self, occ, prc):
        ...
        # Stack features: [B, N, T, 2]
        x = torch.stack([occ, prc], dim=-1)  # [B, N, T, 2]
        x = x.view(B * N, T, 2)  # [B*N, T, 2]

        # LSTM
        lstm_out, _ = self.lstm(x)  # [B*N, T, hidden]

        # TPA: use last hidden state to attend over previous states
        ht = lstm_out[:, -1, :]          # [B*N, hidden]
        H = lstm_out[:, :-1, :]          # [B*N, T-1, hidden]

        # Attention scoring
        score1 = self.W1(H)              # [B*N, T-1, hidden]
        score2 = self.W2(ht).unsqueeze(1)  # [B*N, 1, hidden]
        scores = torch.tanh(score1 + score2)  # [B*N, T-1, hidden]
        attn_weights = self.v(scores).squeeze(-1)  # [B*N, T-1]
        attn_weights = torch.softmax(attn_weights, dim=1)

        # Context vector
        context = torch.bmm(attn_weights.unsqueeze(1), H).squeeze(1)  # [B*N, hidden]

        # Combine
        hx = torch.cat([context, ht], dim=1)  # [B*N, 2*hidden]
        y = self.fc_out(hx)  # [B*N, 1]
        y = y.view(B, N)  # [B, N, 1]
